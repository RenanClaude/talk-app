...
class ChatSerializer(serializers.ModelSerializer):
    ...
    def get_user(self, objChat):
        user = objChat.from_user

        if user.id == self.context["user_id"]:
            user = objChat.to_user
...
class ChatMessagesSerializer(serializers.ModelSerializer):
    ...
    def get_attachment(self, objMessage):
        if objMessage.attachment_code == "FILE":
            file_attachment = FileAttachment.objects.filter(
                id=objMessage.attachment_id
            ).first()

            if not file_attachment:
                return None

            return {"file": FileAttachmentSerializer(file_attachment).data}

        if objMessage.attachment_code == "AUDIO":
            ...
            return {"audio": AudioAttachmentSerializer(audio_attachment).data}
